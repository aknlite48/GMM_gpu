...
int gmm_training(py::array_t<float> pi_k, py::array_t<float> u_k, py::array_t<float> E_k,
                  py::array_t<float> data, int K, int D, int N, float threshold,bool verbose,int max_iter) {
    auto pi_k_ptr = pi_k.mutable_data();
    auto u_k_ptr = u_k.mutable_data();
    auto E_k_ptr = E_k.mutable_data();
    auto data_ptr = data.mutable_data();

    auto iters = GMM_training(pi_k_ptr, u_k_ptr, E_k_ptr, data_ptr, K, D, N, threshold,verbose,max_iter);
    return iters;
}
...
PYBIND11_MODULE(gmm_module, m) {
    ...
    m.def("gmm_training", &gmm_training, "Train the GMM model",
          ...
          py::arg("K"), py::arg("D"), py::arg("N"), py::arg("threshold"),py::arg("verbose"),py::arg("max_iter"));
