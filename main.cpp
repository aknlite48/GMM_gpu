...
int main() {
	...
	M_step_nk_update<<<K,500>>>(d_resp,d_data,d_n_k,d_u_k,N,K,D);
	cudaDeviceSynchronize();

	M_step_uk_update<<<K,500>>>(d_resp,d_data,d_n_k,d_u_k,N,K,D);
	cudaDeviceSynchronize();

	M_step_Ek_update<<<K,500>>>(d_resp,d_data,d_u_k,d_E_k,d_n_k,N,K,D);
